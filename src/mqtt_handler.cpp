...
static String topic_base(uint8_t slave, const String &suffix = "") {
    ...
    if (slave > 0) t += String(slave);
    if (suffix.length() > 0) t += "/" + suffix;
    return t;
}

static String discovery_topic(const char *domain, const String &obj_id) {
    return String("homeassistant/") + domain + "/" +
           String(cfg.hostname) + "_" + obj_id + "/config";
}
...
static String get_manufacturer(Slave_Module *mod) {
    ...
    if (model_name.indexOf("KC868") >= 0) return "KinCony";
    if (model_name.indexOf("Waveshare") >= 0) return "Waveshare";
...
void mqtt_init() {
    ...
    if (mqtt.connect(
        ...
        topic_base(0, "status").c_str(), MQTT_QOS, true, "offline"
    )) {
...
static void mqtt_callback(char *topic, byte *payload, unsigned int length) {
    ...
    for (uint16_t i = 0; i < module_count; i++) {
        if (modules[i].slave_addr == slave) { mod = &modules[i]; break; }
    }
    if (!mod || relay_idx >= mod->model.RELAY_COUNT) return;
        ...
        if (modbus_read_coils(slave, mod->model.RELAY_COUNT, coil_states)) {
            ...
            if (!verified) {
                LOG_E("[MQTT] ⚠ Relay S%d R%d read-back mismatch! Wrote %s, read %s\n",
                    slave, relay_idx, state ? "ON" : "OFF", coil_states[relay_idx] ? "ON" : "OFF");
            }
        }
...
void mqtt_publish_discovery(Slave_Module *mod) {
    ...
    for (uint8_t r = 0; r < mod->model.RELAY_COUNT; r++) {
        ...
        if (area.length() > 0) doc["device"]["suggested_area"] = area;
    ...
    for (uint8_t d = 0; d < mod->model.DI_COUNT; d++) {
        ...
        String click_name = (di_custom.length() > 0) ? di_custom + " Kattintás" : get_entity_name(mod, "DI " + String(d + 1) + " Kattintás");
    ...
    LOG_I("[MQTT] Discovery: S%d (%dR, %dDI, %dDI-Click, 1Addr)\n", 
        mod->slave_addr, mod->model.RELAY_COUNT, mod->model.DI_COUNT, mod->model.DI_COUNT);
...
static void mqtt_click_callback(uint8_t di_idx, uint8_t click_type) {
    if (!click_current_mod) return;
    if (!mqtt.connected()) return;
    
    // Map click_type (1=single, 2=double, 3=triple, 4=long) to ClickType
    ClickType ct;
    switch (click_type) {
        case 1:  ct = CLICK_SINGLE; break;
        case 2:  ct = CLICK_DOUBLE; break;
        case 3:  ct = CLICK_TRIPLE; break;
        case 4:  ct = CLICK_HOLD; break;
        default: ct = CLICK_SINGLE; break;
    }
...
void mqtt_publish_bridge_state() {
    ...
    switch (cfg.active_if) {
        case NET_IF_LAN:  if_name = "LAN"; break;
        case NET_IF_WIFI: if_name = "WiFi"; break;
        default:          if_name = "NONE"; break;
    }
