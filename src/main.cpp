...
static bool alloc_module(Slave_Module **slot) {
    if (module_count >= modules_capacity) {
        ...
        if (new_cap > MODBUS_MAX_SLAVES) new_cap = MODBUS_MAX_SLAVES;
        Slave_Module *new_arr = (Slave_Module*)realloc(modules, new_cap * sizeof(Slave_Module));
        if (!new_arr) {
...
static void update_network() {
    ...
    if (millis() - dbg_timer > 5000) {
        LOG_D("[NET-DBG] lan_en=%d lan_ok=%d wifi_ok=%d active_if=%d net=%d\n",
            cfg.lan_enabled, lan_ok, wifi_ok, cfg.active_if, net_connected);
        dbg_timer = millis();
    }
    
    NetInterface prev_if = cfg.active_if;
...
static void update_status_led() {
    if (cfg.pin_status_led < 0) return;  // LED disabled
    static uint32_t last_blink = 0;
    static bool led_state = false;
    uint32_t interval;
    
    if (!net_connected)           interval = 200;  // Fast blink: no network
    else if (!scanning_done)      interval = 500;  // Medium: scanning
    else if (module_count == 0)   interval = 1500; // Slow: no modules
    else {
...
void module_add_from_scan(uint8_t addr, uint8_t model_id) {
    ...
    if (model_id == 1) strlcpy(mod->model.model_name, "KC868-HA V1", sizeof(mod->model.model_name));
    else if (model_id == 2) strlcpy(mod->model.model_name, "KC868-HA V2", sizeof(mod->model.model_name));
    else snprintf(mod->model.model_name, sizeof(mod->model.model_name), "Unknown (ID=%u)", model_id);
    mod->profile = 0;
    mod->active = true;
...
void scan_modbus_start() {
    ...
    LOG_I("[SCAN] Starting scan %d-%d (profile=%d, timeout=200ms)...\n", 
        cfg.mb_scan_start, cfg.mb_scan_end, cfg.mb_profile);
}

static bool scan_modbus_next() {
    if (!scan_active) return true; // not scanning
    ...
    if (modbus_read_identification(scan_addr, model)) {
        ...
        if (alloc_module(&mod)) {
            ...
            LOG_I("[SCAN] Addr %d: %s (6R, 6DI, FW=%d, SN=%u)\n",
                scan_addr, model.model_name, model.firmware_ver, model.serial_number);
...
static void check_rescan() {
    if (!scanning_done || millis() - last_scan_ms < 60000) return;
    last_scan_ms = millis();
...
void setup() {
    ...
    if (cfg.pin_status_led >= 0) pinMode(cfg.pin_status_led, OUTPUT);
    if (cfg.pin_config_btn >= 0) pinMode(cfg.pin_config_btn, INPUT_PULLUP);
    ...
    switch (setup_stage) {
        ...
        case 4: {
            ...
            if (saved_count > 0) {
                ...
                if (cfg.virtual_module) { module_add_virtual(); }
                scanning_done = true;
                const char* if_name = cfg.active_if == NET_IF_LAN ? "LAN" : 
                                      cfg.active_if == NET_IF_WIFI ? "WiFi" : "NONE";
