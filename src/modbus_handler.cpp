...
{
    if (cfg.pin_rs485_de >= 0)
        digitalWrite(cfg.pin_rs485_de, HIGH); // Enable transmit
...
static void post_transmission()
{
    ...
    if (cfg.pin_rs485_de >= 0)
        digitalWrite(cfg.pin_rs485_de, LOW); // Back to receive
}
...
{
    if (addr == current_slave)
        return; // Already addressing this slave
...
ModbusRawResult modbus_raw_request(
        uint8_t slave, uint8_t func, uint16_t start, uint16_t count_or_value, uint16_t count2, const uint16_t *values)
{
    ...
    {
    case 0x01:
        result.status = node.readCoils(start, count_or_value);
        break;
    case 0x02:
        result.status = node.readDiscreteInputs(start, count_or_value);
        break;
    case 0x03:
        result.status = node.readHoldingRegisters(start, count_or_value);
        break;
    case 0x04:
        result.status = node.readInputRegisters(start, count_or_value);
        break;
    case 0x05:
        result.status = node.writeSingleCoil(start, count_or_value);
        break;
    case 0x06:
        result.status = node.writeSingleRegister(start, count_or_value);
        break;
    case 0x0F:
    {
...
bool modbus_read_identification(uint8_t slave, HA_Model &model)
{
    ...
    {
    case 1:
        strncpy(model.model_name, "KC868-HA1", sizeof(model.model_name));
        break;
    case 2:
        strncpy(model.model_name, "KC868-HA2", sizeof(model.model_name));
        break;
    case 4:
        strncpy(model.model_name, "KC868-HA4", sizeof(model.model_name));
        break;
    case 8:
        strncpy(model.model_name, "KC868-HA8", sizeof(model.model_name));
        break;
    case 16:
        strncpy(model.model_name, "KC868-HA16", sizeof(model.model_name));
        break;
    default:
...
bool modbus_read_discrete_inputs(uint8_t slave, uint8_t count, bool *states)
{
    ...
    // Validate byte_count from response (Modbus spec: ceil(count/8))
    uint8_t byte_count = node.getResponseBuffer(0);
    uint8_t expected_bc = (count + 7) / 8;
    if (byte_count != expected_bc)
    {
        LOG_I("[MB] FC02 byte_count mismatch: got %d, expected %d\n", byte_count, expected_bc);
    }
...
bool modbus_read_coils(uint8_t slave, uint8_t count, bool *states)
{
    ...
    if (byte_count != expected_bc)
    {
        LOG_I("[MB] FC01 byte_count mismatch: got %d, expected %d\n", byte_count, expected_bc);
    }
    for (uint8_t i = 0; i < count && i < 16; i++)
    {
        uint8_t byte_idx = 1 + (i / 8);
