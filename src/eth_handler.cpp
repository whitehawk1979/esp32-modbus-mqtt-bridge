...
public:
    EthServer(uint16_t port) : EthernetServer(port) {}
...
void eth_init() {
    ...
    const char* type_name = cfg.lan_type == 0 ? "W5500-SPI" : 
                           cfg.lan_type == 1 ? "LAN8720-RMII" : 
                           "IP101-EMAC";
...
void eth_loop() {
    ...
    if (cfg.lan_type == 0 && !lan_started && !w5500_task_done) {
        ...
        if (!task_created) {
            ...
            if (millis() - boot_time < 3000) return;
...
String eth_get_ip() {
    if (!lan_connected || !lan_started) return "0.0.0.0";
...
static void eth_handle_api_lan(EthernetClient& client) {
    ...
    client.printf(
        ...
        cfg.lan_enabled ? "true" : "false", cfg.lan_type,
        lan_started ? "true" : "false",
        lan_connected ? "true" : "false",
        Ethernet.hardwareStatus() == EthernetW5500 ? "W5500" : "NO_HW",
        Ethernet.linkStatus() == LinkON ? "ON" : "OFF",
        ...
        cfg.pin_eth_rst, cfg.pin_eth_cs, cfg.pin_eth_sclk,
        cfg.pin_eth_miso, cfg.pin_eth_mosi, cfg.pin_eth_int
    );
    client.stop();
}
#endif
...
void eth_web_loop() {
    if (!lan_started || !lan_connected) return;
    ...
    while (client.connected() && millis() - t < 500) {
        ...
        if (hdr.length() <= 1) break;  // empty line = end of headers
    }
    ...
    if (path == "/api/status") {
        ...
        if (path == "/api/reboot") {
            ...
            if (qi > 0) {
                ...
                while (pos < (int)qs.length()) {
                    ...
                    if (key == "ssid")     nv.putString(NV_KEY_WIFI_SSID, val);
                    else if (key == "wpass")    nv.putString(NV_KEY_WIFI_PASS, val);
                    else if (key == "hostname") nv.putString(NV_KEY_HOSTNAME, val);
                    else if (key == "mhost")   nv.putString(NV_KEY_MQTT_HOST, val);
                    else if (key == "mport")   nv.putUInt(NV_KEY_MQTT_PORT, val.toInt());
                    else if (key == "muser")   nv.putString(NV_KEY_MQTT_USER, val);
                    else if (key == "mpass")   nv.putString(NV_KEY_MQTT_PASS, val);
                    else if (key == "mpfx")    nv.putString(NV_KEY_MQTT_PREFIX, val);
                    else if (key == "lanen")   nv.putBool(NV_KEY_ETH_EN, val == "1");
                    else if (key == "landhcp") nv.putBool(NV_KEY_ETH_DHCP, val == "1");
                    else if (key == "hadisc")  nv.putBool(NV_KEY_HA_DISC, val == "1");
                    else if (key == "vmod")    nv.putBool(NV_KEY_VIRTUAL_MOD, val == "1");
                    else if (key == "mbprof")  nv.putUChar(NV_KEY_MB_PROFILE, (uint8_t)val.toInt());
                    else if (key == "mbpoll")  nv.putUInt(NV_KEY_MB_POLL_MS, val.toInt());
                    else if (key == "mstart")  nv.putUChar(NV_KEY_MB_START, (uint8_t)val.toInt());
                    else if (key == "mend")    nv.putUChar(NV_KEY_MB_END, (uint8_t)val.toInt());
                    else if (key == "tcpen")   nv.putBool(NV_KEY_TCP_EN, val == "1");
                    else if (key == "tcpp")    nv.putUInt(NV_KEY_TCP_PORT, val.toInt());
            ...
            char ssid[64] = {}; nv.getString(NV_KEY_WIFI_SSID, ssid, sizeof(ssid));
            char wpass[64] = {}; nv.getString(NV_KEY_WIFI_PASS, wpass, 4); // only first 3 chars for security
            char mhost[64] = {}; nv.getString(NV_KEY_MQTT_HOST, mhost, sizeof(mhost));
            uint16_t mport = nv.getUInt(NV_KEY_MQTT_PORT, 0);
            bool lanen = nv.getBool(NV_KEY_ETH_EN, false);
            char hostname[64] = {}; nv.getString(NV_KEY_HOSTNAME, hostname, sizeof(hostname));
            nv.end();
            char buf[512];
            snprintf(buf, sizeof(buf),
                "{\"ssid\":\"%s\",\"wpass_ok\":%s,\"mhost\":\"%s\",\"mport\":%u,\"lanen\":%s,\"hostname\":\"%s\"}",
                ssid, (wpass[0] ? "\"true\"" : "\"false\""), mhost, mport,
                lanen ? "true" : "false", hostname);
        ...
        // ── /api/export — Full config export (all NVRAM keys + module names) ──
            ...
            const char* str_keys[] = {
                NV_KEY_WIFI_SSID, NV_KEY_WIFI_PASS, NV_KEY_HOSTNAME,
            ...
            const char* uint_keys[] = {
                NV_KEY_MQTT_PORT, NV_KEY_MB_BAUD, NV_KEY_MB_POLL_MS,
            ...
            const char* uint_names[] = {
                "mport","mbaud","mbpoll","mstart","mend","mpar",
                "tcpp","prx","ptx","pde","pled","pbtn",
                "pemosi","pemiso","pesclk","pecs","peint","perst",
                "mbprof","mbcoil","mbdi"
            };
            ...
            for (uint8_t i = 0; i < mlist_n && i < 16; i++) {
                char akey[12]; snprintf(akey, sizeof(akey), "%s%u", NV_KEY_MOD_ADDR, i);
                if (i > 0) json += ",";
                json += String(nv.getUChar(akey, 0));
            }
            json += "],\"mlist_m\":[";
            for (uint8_t i = 0; i < mlist_n && i < 16; i++) {
                char mkey[12]; snprintf(mkey, sizeof(mkey), "%s%u", NV_KEY_MOD_MODEL, i);
                if (i > 0) json += ",";
            ...
            for (uint8_t mi = 0; mi < mlist_n && mi < 16; mi++) {
                char akey[12]; snprintf(akey, sizeof(akey), "%s%u", NV_KEY_MOD_ADDR, mi);
                uint8_t addr = nv.getUChar(akey, 0);
                if (addr == 0) continue;

                // Get model to know relay/DI counts
                char mkey[12]; snprintf(mkey, sizeof(mkey), "%s%u", NV_KEY_MOD_MODEL, mi);
                ...
                if (mid == 200) { max_r = 6; max_di = 6; }  // KinCony F16
                else if (mid == 100) { max_r = 8; max_di = 8; }  // KinCony A8
                else if (mid == 201) { max_r = 16; max_di = 16; } // KinCony A16
                else { max_r = 6; max_di = 6; } // default

                if (!first_mod) json += ",";
                first_mod = false;

                char mod_key[8]; snprintf(mod_key, sizeof(mod_key), "%u", addr);
                json += "\"" + String(mod_key) + "\":{";

                // Module name (mn<addr>)
                char mnk[8]; snprintf(mnk, sizeof(mnk), "mn%u", addr);
                char mnv[64] = {}; nv.getString(mnk, mnv, sizeof(mnv));
                String escaped_mn = String(mnv);
                escaped_mn.replace("\\", "\\\\"); escaped_mn.replace("\"", "\\\"");
                json += "\"name\":\"" + escaped_mn + "\"";

                // HA name (hn<addr>)
                char hnk[8]; snprintf(hnk, sizeof(hnk), "hn%u", addr);
                char hnv[64] = {}; nv.getString(hnk, hnv, sizeof(hnv));
                String escaped_hn = String(hnv);
                escaped_hn.replace("\\", "\\\\"); escaped_hn.replace("\"", "\\\"");
                json += ",\"ha_name\":\"" + escaped_hn + "\"";

                // Area (ar<addr>)
                char ark[10]; snprintf(ark, sizeof(ark), "ar%u", addr);
                char arv[64] = {}; nv.getString(ark, arv, sizeof(arv));
                String escaped_ar = String(arv);
                escaped_ar.replace("\\", "\\\\"); escaped_ar.replace("\"", "\\\"");
                json += ",\"area\":\"" + escaped_ar + "\"";

                // Relay names
                json += ",\"relays\":[";
                for (uint8_t r = 0; r < max_r; r++) {
                    char rnk[12]; snprintf(rnk, sizeof(rnk), "rn%u_%u", addr, r);
                    char rnv[64] = {}; nv.getString(rnk, rnv, sizeof(rnv));
                    String escaped_rn = String(rnv);
                    escaped_rn.replace("\\", "\\\\"); escaped_rn.replace("\"", "\\\"");
                    if (r > 0) json += ",";
                    json += "\"" + escaped_rn + "\"";
                }
                json += "]";
                ...
                for (uint8_t d = 0; d < max_di; d++) {
                    char dnk[12]; snprintf(dnk, sizeof(dnk), "dn%u_%u", addr, d);
                    char dnv[64] = {}; nv.getString(dnk, dnv, sizeof(dnv));
                    String escaped_dn = String(dnv);
                    escaped_dn.replace("\\", "\\\\"); escaped_dn.replace("\"", "\\\"");
                    if (d > 0) json += ",";
                    json += "\"" + escaped_dn + "\"";
                }
                json += "]";
        ...
        // ── /api/import — Full config import from JSON query param body ──
            ...
            if (qi < 0) {
                ...
                while (pos < (int)qs.length()) {
                    ...
                    if (key == "fw_version" || key.startsWith("mlist_") || key == "modules") continue;

                    // String config keys → putString
                    if (key == "ssid")  { nv.putString(NV_KEY_WIFI_SSID, raw_val); restored++; }
                    else if (key == "wpass") { nv.putString(NV_KEY_WIFI_PASS, raw_val); restored++; }
                    else if (key == "hostname") { nv.putString(NV_KEY_HOSTNAME, raw_val); restored++; }
                    else if (key == "mhost") { nv.putString(NV_KEY_MQTT_HOST, raw_val); restored++; }
                    else if (key == "muser") { nv.putString(NV_KEY_MQTT_USER, raw_val); restored++; }
                    else if (key == "mpass") { nv.putString(NV_KEY_MQTT_PASS, raw_val); restored++; }
                    else if (key == "mpfx")  { nv.putString(NV_KEY_MQTT_PREFIX, raw_val); restored++; }
                    // UInt config keys → putUInt
                    else if (key == "mport")  { nv.putUInt(NV_KEY_MQTT_PORT, raw_val.toInt()); restored++; }
                    else if (key == "mbaud")  { nv.putUInt(NV_KEY_MB_BAUD, raw_val.toInt()); restored++; }
                    else if (key == "mbpoll") { nv.putUInt(NV_KEY_MB_POLL_MS, raw_val.toInt()); restored++; }
                    else if (key == "mstart") { nv.putUInt(NV_KEY_MB_START, raw_val.toInt()); restored++; }
                    else if (key == "mend")   { nv.putUInt(NV_KEY_MB_END, raw_val.toInt()); restored++; }
                    else if (key == "mpar")   { nv.putUInt(NV_KEY_MB_PARITY, raw_val.toInt()); restored++; }
                    else if (key == "tcpp")   { nv.putUInt(NV_KEY_TCP_PORT, raw_val.toInt()); restored++; }
                    else if (key == "prx")    { nv.putUInt(NV_KEY_PIN_RS485_RX, raw_val.toInt()); restored++; }
                    else if (key == "ptx")    { nv.putUInt(NV_KEY_PIN_RS485_TX, raw_val.toInt()); restored++; }
                    else if (key == "pde")    { nv.putUInt(NV_KEY_PIN_RS485_DE, raw_val.toInt()); restored++; }
                    else if (key == "pled")   { nv.putUInt(NV_KEY_PIN_LED, raw_val.toInt()); restored++; }
                    else if (key == "pbtn")   { nv.putUInt(NV_KEY_PIN_BTN, raw_val.toInt()); restored++; }
                    else if (key == "pemosi") { nv.putUInt(NV_KEY_PIN_ETH_MOSI, raw_val.toInt()); restored++; }
                    else if (key == "pemiso") { nv.putUInt(NV_KEY_PIN_ETH_MISO, raw_val.toInt()); restored++; }
                    else if (key == "pesclk") { nv.putUInt(NV_KEY_PIN_ETH_SCLK, raw_val.toInt()); restored++; }
                    else if (key == "pecs")   { nv.putUInt(NV_KEY_PIN_ETH_CS, raw_val.toInt()); restored++; }
                    else if (key == "peint")  { nv.putUInt(NV_KEY_PIN_ETH_INT, raw_val.toInt()); restored++; }
                    else if (key == "perst")  { nv.putUInt(NV_KEY_PIN_ETH_RST, raw_val.toInt()); restored++; }
                    else if (key == "mbprof") { nv.putUInt(NV_KEY_MB_PROFILE, raw_val.toInt()); restored++; }
                    else if (key == "mbcoil") { nv.putUInt(NV_KEY_MB_REG_COIL, raw_val.toInt()); restored++; }
                    else if (key == "mbdi")   { nv.putUInt(NV_KEY_MB_REG_DI, raw_val.toInt()); restored++; }
                    // Bool config keys → putBool
                    else if (key == "ethen")  { nv.putBool(NV_KEY_ETH_EN, raw_val == "true" || raw_val == "1"); restored++; }
                    else if (key == "edhcp")  { nv.putBool(NV_KEY_ETH_DHCP, raw_val == "true" || raw_val == "1"); restored++; }
                    else if (key == "wdhcp")  { nv.putBool(NV_KEY_WIFI_DHCP, raw_val == "true" || raw_val == "1"); restored++; }
                    else if (key == "tcpen")  { nv.putBool(NV_KEY_TCP_EN, raw_val == "true" || raw_val == "1"); restored++; }
                    else if (key == "hadisc") { nv.putBool(NV_KEY_HA_DISC, raw_val == "true" || raw_val == "1"); restored++; }
                    else if (key == "vmod")   { nv.putBool(NV_KEY_VIRTUAL_MOD, raw_val == "true" || raw_val == "1"); restored++; }
                    else if (key == "mtls")   { nv.putBool(NV_KEY_MQTT_TLS, raw_val == "true" || raw_val == "1"); restored++; }
                    // UInt8 keys → putUChar
                    else if (key == "etype")    { nv.putUChar(NV_KEY_ETH_TYPE, (uint8_t)raw_val.toInt()); restored++; }
                    else if (key == "wifimode") { nv.putUChar(NV_KEY_WIFI_MODE, (uint8_t)raw_val.toInt()); restored++; }
                    else if (key == "nprio")    { nv.putUChar(NV_KEY_NET_PRIO, (uint8_t)raw_val.toInt()); restored++; }
                    // Module names (mn<addr>, hn<addr>, ar<addr>, rn<addr>_<idx>, dn<addr>_<idx>)
                    else if (key.startsWith("mn")) { nv.putString(key.c_str(), raw_val); restored++; }
                    else if (key.startsWith("hn")) { nv.putString(key.c_str(), raw_val); restored++; }
                    else if (key.startsWith("ar")) { nv.putString(key.c_str(), raw_val); restored++; }
                    else if (key.startsWith("rn")) { nv.putString(key.c_str(), raw_val); restored++; }
                    else if (key.startsWith("dn")) { nv.putString(key.c_str(), raw_val); restored++; }
                    // Module list
                    else if (key == "mlist_n") { nv.putUChar(NV_KEY_MOD_LIST_N, (uint8_t)raw_val.toInt()); restored++; }
                    else if (key.startsWith("mlist_a")) { nv.putUChar(key.c_str(), (uint8_t)raw_val.toInt()); restored++; }
                    else if (key.startsWith("mlist_m")) { nv.putUChar(key.c_str(), (uint8_t)raw_val.toInt()); restored++; }
                }
