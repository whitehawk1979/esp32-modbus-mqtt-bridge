...
uint8_t W5100Class::init(void)
{
	...
	//SPI.begin();  // Removed: SPI must be initialized with W5500 pins BEFORE Ethernet.begin()
	// The caller (eth_handler) already called SPI.begin(sclk, miso, mosi, cs) with correct pins
	// Calling SPI.begin() without params would reset to FSPI defaults (wrong MISO/SCK for W5500)
	initSS();
	resetSS();
